...
#define ITERS 10
  ...
  unsigned long ctlwwm = 0x80145080; /* Write with MPC */
  unsigned long ctlww  = 0x00145080;
  unsigned long ctlwrm = 0x80045080; /* Read with MPC */
  unsigned long ctlwr  = 0x00045080;
  unsigned int j,k;
  /*--------------------------------*/
  ...
  */
  ...
  {  
    ...
    for (k=0; k<NWRITES; k++)
    {
      if ((read_s[k].indat & 0xFFFF0000) != (write_s[k].outdat & 0xFFFF0000))
